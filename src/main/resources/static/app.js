...
async function waitHlsPlayable(item, sessionId) {
    while (sessionId === hlsViewerSession) {
        const response = await secureFetch(item.hlsStatusUrl);
...
function showHlsProgress(progress = 0) {
    let widget = document.getElementById("hlsProgressWidget");

    if (!widget) {
        widget = document.createElement("div");
        widget.id = "hlsProgressWidget";
        widget.className = "hls-progress-widget";
        viewer.appendChild(widget);
    }

    widget.innerHTML = `
        <div class="hls-progress-ring" style="--progress:${progress}%">
            <div class="hls-progress-inner">${progress}%</div>
        </div>
    `;
}
function hideHlsProgress() {
    const widget = document.getElementById("hlsProgressWidget");
...
function startHlsProgressPolling(item, sessionId) {
    hideHlsProgress();
    showHlsProgress(0);
    ...
    hlsProgressTimer = setInterval(async () => {
        if (sessionId !== hlsViewerSession) {
            hideHlsProgress();
            return;
        }

        const response = await secureFetch(progressUrl);
...
/!*function playHlsInViewer(playlistUrl, item) {
    ...
    if (window.Hls && Hls.isSupported()) {
        ...
        hls.on(Hls.Events.ERROR, async (event, data) => {
            ...
            const sessionId = hlsViewerSession;
            ...
            if (sessionId !== hlsViewerSession) return;

            const statusResponse = await secureFetch(item.hlsStatusUrl);
...
async function shutdownHlsBeforeLeave() {
    hlsViewerSession++;
...
`;
        ...
        /!*if ((lower.endsWith(".insv") || lower.endsWith(".lrv")) && !item.hlsPrepareUrl) {
            item.hlsSupported = true;
            item.hlsPrepareUrl =
                `/api/video/hls/prepare?path=${encodeURIComponent(item.relativePath)}`;
            item.hlsStatusUrl =
        ...
        /!*if (lower.endsWith(".insv") || lower.endsWith(".lrv")) {
            item.hlsPrepareUrl = item.hlsPrepareUrl || `/api/video/hls/prepare?path=${encodeURIComponent(item.relativePath)}`;
            item.hlsStatusUrl = item.hlsStatusUrl || `/api/video/hls/status?path=${encodeURIComponent(item.relativePath)}`;
